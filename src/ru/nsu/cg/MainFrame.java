...
import ru.nsu.fit.g13204.Khenkina.view.StatusBar;
...
public class MainFrame extends JFrame {
	 ...
	 */
	public JMenuItem createMenuItem(String title, String tooltip, int mnemonic, String icon, Controller controller, String actionMethod) throws SecurityException, NoSuchMethodException {
		JMenuItem item = new JMenuItem(title);
		return configureMenuItem(item, title, tooltip, mnemonic, icon, controller, actionMethod);
	}

	public JMenuItem createCheckBoxMenuItem(String title, String tooltip, int mnemonic, String icon, Controller controller, String actionMethod) throws SecurityException, NoSuchMethodException
	{
		JCheckBoxMenuItem item = new JCheckBoxMenuItem(title);
		return configureMenuItem(item, title, tooltip, mnemonic, icon, controller, actionMethod);
	}

	public JMenuItem createRadioButtonMenuItem(String title, String tooltip, int mnemonic, String icon, Controller controller, String actionMethod) throws SecurityException, NoSuchMethodException
	{
		JRadioButtonMenuItem item = new JRadioButtonMenuItem(title);
		return configureMenuItem(item, title, tooltip, mnemonic, icon, controller, actionMethod);
	}

	private JMenuItem configureMenuItem(JMenuItem item, String title, String tooltip, int mnemonic, String icon, Controller controller, String actionMethod) throws SecurityException, NoSuchMethodException
	{
		...
		Method method;
		if(controller!= null){
			method = controller.getClass().getMethod(actionMethod);
		}
		else{
	...
	{
		return createMenuItem(title, tooltip, mnemonic, null, null, actionMethod);
	}
	 ...
	 */
	public JMenuItem addMenuItem(String title, String tooltip, int mnemonic, String icon, Controller controller, String actionMethod, StatusBar statusBar) throws SecurityException, NoSuchMethodException
	{
		MenuElement element = getParentMenuElement(title);
		if(element == null)
			throw new InvalidParameterException("Menu path not found: "+title);
		JMenuItem item = createMenuItem(getMenuPathName(title), tooltip, mnemonic, icon, controller, actionMethod);
		addToStatusBar(item, tooltip, statusBar);
	...
	private void addToStatusBar(JMenuItem item, String tooltip, StatusBar statusBar){
	...
	private void addToStatusBar(AbstractButton button, String tooltip, StatusBar statusBar){
	...
	public JMenuItem addCheckBoxMenuItem(String title, String tooltip, int mnemonic, String icon, Controller controller, String actionMethod, StatusBar statusBar) throws SecurityException, NoSuchMethodException
	{
		MenuElement element = getParentMenuElement(title);
		if(element == null)
			throw new InvalidParameterException("Menu path not found: "+title);
		JMenuItem item = createCheckBoxMenuItem(getMenuPathName(title), tooltip, mnemonic, icon, controller, actionMethod);
		addToStatusBar(item, tooltip, statusBar);
		if(element instanceof JMenu)
			((JMenu)element).add(item);
		else if(element instanceof JPopupMenu)
	...
	{
		addMenuItem(title, tooltip, mnemonic, null, null, actionMethod, null);
	}
	 ...
	 * @param menuPath - path to menu item to create toolbar button from
	 */
	public JButton addToolBarButton(String menuPath, StatusBar statusBar)
	{
		JButton button = createToolBarButton(menuPath);
		toolBar.add(button);
		addToStatusBar(button, button.getToolTipText(), statusBar);
		return button;
	}
	...
	public JToggleButton addToolBarToggleButton(String menuPath, StatusBar statusBar)
	{
	 ...
	 */
	public File getSaveFileName(String extension, String description, String directoryName)
	{
		return FileUtils.getSaveFileName(this, extension, description, new File(directoryName));
	}
	 ...
	 */
	public File getOpenFileName(String extension, String description, String directoryName)
	{
		return FileUtils.getOpenFileName(this, extension, description, new File(directoryName));
	}
